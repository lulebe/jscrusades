...
{
   trace("AiWishlistCreate()");
   AiWishlistReset();
}
function AiWishlistDestroy()
{
   trace("AiWishlistDestroy()");
}
function AiWishlistReset()
{
   trace("AiWishlistReset()");
   i = 0;
...
function AiWishlistGetRandomProfile()
{
   ...
   return aiWishlist_profiles[myRnd(0,aiWishlist_counter - 1)];
}
...
{
   trace("AiInfluencePrepareMap_Fortify()");
   AiInfluenceClearFortify();
   for(i in World.unitList)
   {
      unit = World.unitList[i];
      if(player.areEnemy(unit))
      {
         if(World.DataUnitsCanCaptureCity(World.UnitsGetProfile(unit)))
         {
            World.createMovement(unit,false);
            var _loc2_ = 0;
            while(_loc2_ < World.rows)
            {
               ...
               {
                  if(World.map.movement[_loc2_][_loc1_] > 0)
                  {
...
function AiInfluencePrepareMap_Supply(item)
{
   AiInfluenceClearScore();
   AiInfluenceClearMarker();
   var _loc6_ = null;
   var _loc4_ = null;
   var _loc7_ = World.UnitsGetProfile(item);
   var _loc9_ = World.DataUnitsGetCategory(_loc7_);
   var _loc11_ = undefined;
   var _loc10_ = undefined;
   var _loc8_ = false;
   var _loc3_ = 0;
   while(_loc3_ < World.rows)
   {
      var _loc2_ = 0;
      while(_loc2_ < World.cols)
      {
         score = 0;
         _loc4_ = World.getCity(_loc3_,_loc2_);
         _loc6_ = World.map.units[_loc3_][_loc2_];
         _loc8_ = false;
         if(_loc4_ != null)
         {
            if(_loc4_.getPlayer() == null && World.DataUnitsCanCaptureCity(_loc7_))
            {
               if(_loc6_ == item)
               {
            ...
            else if(World.CitiesCanSupplyUnit(_loc4_,item))
            {
               ...
               while(neighbour < 4)
               {
                  var _loc1_ = World.NeighboursSelect(neighbour,_loc3_,_loc2_);
                  if(_loc1_ != null)
                  {
                     if(World.IsPositionBlocked(_loc1_.row,_loc1_.col,World.UnitsGetProfile(item)) == false)
                     {
...
function AiInfluencePrepareMap_Fight(item)
{
   AiInfluenceClearScore();
   AiInfluenceClearMarker();
   AiInfluenceCreateThreat(item);
   var _loc8_ = null;
   var _loc11_ = null;
   ...
   while(_loc4_ < World.rows)
   {
      ...
      while(_loc3_ < World.cols)
      {
         ...
         _loc11_ = World.getCity(_loc4_,_loc3_);
         if(_loc11_ != null)
         {
            _loc2_ = AiInfluenceGetScoreForCity(item,_loc11_);
         ...
         _loc8_ = World.map.units[_loc4_][_loc3_];
         if(_loc8_ != null)
         {
            if(World.PlayerAreEnemies(_loc8_.getPlayer(),item.getPlayer()))
            {
               _loc9_ = AiInfluenceGetScoreForEnemyUnit(item,_loc8_);
               ...
               while(neighbour < 4)
               {
                  var _loc1_ = World.NeighboursSelect(neighbour,_loc4_,_loc3_);
                  _loc2_ = _loc9_;
                  if(_loc1_ != null)
                  {
                     var _loc6_ = World.map.units[_loc1_.row][_loc1_.col];
                     _loc7_ = AiInfluenceGetScoreForTerrain(item,_loc8_,_loc1_.row,_loc1_.col);
                     if(_loc7_ != Ai_InvalidValue)
                     {
                        ...
                        else if(World.PlayerAreEnemies(_loc6_.getPlayer(),item.getPlayer()))
                        {
         ...
         if(item.getMovementCost(World.map.terrain[_loc4_][_loc3_].name) == GameMovement_BlockedField)
         {
...
function AiInfluenceGetScoreForCity(item, city)
{
   ...
   trace("AiInfluenceGetScoreForCity() ERROR!!!");
}
...
function AiToolsBattle(item)
{
   ...
   trace("BattleDescription: " + World.BattleDescription[0]);
   var _loc1_ = AiToolsSearchBestBattleDescription(item,false);
   trace(_loc1_[0] + " " + _loc1_[1] + " " + _loc1_[2].type);
   if(_loc1_ != null)
   {
...
{
   trace("AiSystemCreate( world )");
   World = world;
   AiBuyUnitsCreate();
}
function AiSystemDestroy()
{
   trace("AiSystemDestroy()");
   AiBuyUnitsDestroy();
}
...
{
   trace("AiSystemExecute_StandardUnits()");
   UnitCounter = 0;
...
var AiWishlist_Max = 1000;
var aiWishlist_counter = 0;
var aiWishlist_profiles = new Array(AiWishlist_Max);
